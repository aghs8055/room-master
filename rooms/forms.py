...
class RequestApprovalForm(forms.ModelForm):
    ...
    def clean_request(self):
        request = Request.objects.filter(
            id=self.cleaned_data["request"], status=Request.Status.PENDING
        ).first()
        if request:
            return request
        else:
            raise forms.ValidationError(
                {"request_id": "There is no request with this ID."}
            )
    ...
    def get_time_diff(self, start_time, end_time):
        diff = datetime.combine(datetime.today(), end_time) - datetime.combine(
    ...
    def clean(self):
        ...
        if request.duration and request.duration != self.get_time_diff(
            cleaned_data.get("start_time"), cleaned_data.get("end_time")
        ):
            errors[
                "end_time"
            ] = f"End time must be {request.duration} minutes after start time."

        if not request.duration and request.start_time != cleaned_data.get(
            "start_time"
        ):
        ...
        if (
            Reservation.objects.filter(
                room=cleaned_data.get("room"), date=cleaned_data.get("date")
            )
            .exclude(
                start_time__gt=cleaned_data.get("end_time"),
                end_time__lt=cleaned_data.get("start_time"),
            )
            .exists()
        ):
        ...
        if not errors and not cleaned_data.get("force"):
            overlap_requests = (
                Request.objects.filter(status=Request.Status.PENDING)
                .exclude(id=request.id)
                .filter(date=self.cleaned_data["date"])
                .exclude(
                    start_time__gt=self.cleaned_data["end_time"],
                    end_time__lt=self.cleaned_data["start_time"],
                )
            )
